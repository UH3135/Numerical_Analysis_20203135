...
if __name__ == '__main__':
    ...
    sx, ex, ea2, cnt = falsePosition(start, end, f, stand)
    print(f"가위치법: [{sx}, {ex}] 사이에 근이 존재, 상대오차는 {ea2}, 총 {cnt}번 반복했습니다.")

    if ea1 > ea2:
        print(f"이분법이 가위치법보다 {ea1 - ea2}만큼 더 오차가 발생했습니다")
    elif ea1 < ea2:
        print(f"가위치법이 이분법보다 {ea2 - ea1}만큼 더 오차가 발생했습니다")
    else:
        print("이분법, 가위치법 둘 다 같은 횟수만큼 오차가 났습니다.")
